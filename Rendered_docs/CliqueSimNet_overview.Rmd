---
title: 'SimNetClique: algorithmic overview'
author: "Piotr Stomma"
date: "2025-01-24"
output:
  html_document:
...
We discuss technical details of SimNetClique which are not obvious only from description of main steps of the algorithm and provide some illustratory examples, which might be helpful for understanding but are too detailed for the main description.

## Purpose of the algorithm

Algorithm is supposed to find clusters according to pairwise similarities between objects. 
Name stands for CliqueSimNet -- clique based similarity network clustering and describes its main mechanism
...
To find optimal threshold by `compexity` of a clique partition, we subject $G_{\mathbf{S}}$ to candidate threshold $t$ and calculate complexity for cliques found on this $t$ (see docs of `cliquePartitioneR::thr_optimizer`.
...
Below plot is ordered by the clique label and show "degree" of each clique in clique similarity graphs: with and without null model.
which is determined by the order in which it was built by our clique partitioning.
...
#par(mfrow=c(2,1))
{#options(warn=FALSE)
  plot(colSums(CS), type="l", xlab="degree of each clique",
       ylab="")
...
{#options(warn=FALSE)
  plot( sizes_label_ordered,
               colSums(CS), type="p", xlab="degree of each clique",
       ylab="",pch=16)
...
We focus on the `MSTDunn` quality function (defined in terms of distances) suited for problems where non-oblique clusters are meaningful:
...
#                hausdorff_q =1)) #default value is max 
